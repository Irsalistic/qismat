...
def load_bonds(path: Path) -> set[str]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not line or line.startswith("#") or line.startswith("["):
            continue
        try:
            bonds.add(normalize_bond(line.split()[0].split(",")[0]))
        except ValueError:
            bonds.update(extract_bonds_from_text(line))
...
def save_bonds(path: Path, bonds: set[str]) -> None:
    header = (
        "# Prize bond numbers (one per line)\n"
        "# Import bulk: qismat --import your_file.csv\n"
    )
    body = "\n".join(sorted(bonds))
